  ...
  return (
    <>
      <section className="overflow-hidden" ref={container}>
        <motion.div
          className="relative w-screen h-[180svh] overflow-hidden"
          style={{ y }}
        >
          <div className="absolute inset-0 w-screen h-[180svh]" ref={ref}>
            <div className="grid grid-cols-3 gap-2">
              {works.slice(0, 12).map((work, i) => (
                <Card key={i} work={work} index={i} />
              ))}
            </div>
          </div>
        </motion.div>
      ...
      <AnimatePresence mode="wait">
        {inView && (
          ...
          >
            <Link
              href="/works"
              className="group col-start-3 inline-block cursor-pointer text-s pointer-events-auto"
            >
              <span className="relative text-s  text-[62px] tracking-[-0.03em] leading-none max-lg:text-[48px] max-md:text-[42px]">
                See All Works
              ...
              <span className="relative font-general text-s/75 leading-none tracking-[-0.05em] uppercase -top-8 left-2 max-lg:-top-6 max-lg:text-[14px]">
                (20)
              </span>
            </Link>
          </motion.div>
